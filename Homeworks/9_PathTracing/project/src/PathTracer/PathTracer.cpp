...
rgbf PathTracer::Shade(const IntersectorClosest::Rst& intersection, const vecf3& wo, bool last_bounce_specular) const {
	...
	vecf3 x = intersection.pos.cast_to<vecf3>();
	scene->Each([=, &L_dir](const Cmpt::Light* light, const Cmpt::L2W* l2w, const Cmpt::SObjPtr* ptr) {
		...
		if (sample_light_rst.is_infinity) {
			...
			bool visible = IntersectorVisibility::Instance().Visit(&bvh, rayf3(intersection.pos, -sample_light_rst.n.cast_to<vecf3>()));
			if (visible)
			{
				float cosx2yn = normalf::cos_theta(-sample_light_rst.n, intersection.n);
				//Geo = cosx2yn*cosx2yn;
				L_dir += cosx2yn * cosx2yn * BRDF(intersection, -sample_light_rst.n.cast_to<vecf3>(), wo) * sample_light_rst.L / sample_light_rst.pd;
			}
		}
		else {
			// TODO: L_dir of area light
			vecf3 y = sample_light_rst.x.cast_to<vecf3>();
			vecf3 x2y = y - x;
			vecf3 dirx2y = x2y.normalize();
			if (vecf3::dot(dirx2y, sample_light_rst.n.cast_to<vecf3>()) < 0.f)
			{
				bool visible = IntersectorVisibility::Instance().Visit(&bvh, rayf3(intersection.pos, dirx2y, EPSILON<float>, x2y.norm() - EPSILON<float>));
				if (visible)
				{
					float cosx2yn = vecf3::dot(dirx2y, intersection.n.cast_to<vecf3>());
					Geo = cosx2yn * cosx2yn / x2y.norm2();
					L_dir += Geo * BRDF(intersection, dirx2y, wo) * sample_light_rst.L / sample_light_rst.pd;
				}
			}
		}
	});
	...
	auto [wi, pd] = SampleBRDF(intersection, wo);
	...
	if (isnan(Li[0]) || isnan(Li[1]) || isnan(Li[2])) return L_dir;	//In case of NAN.
	L_indir += BRDF(intersection, wi, wo) * Li * abs(vecf3::cos_theta(wi, intersection.n.cast_to<vecf3>())) / pd;
